...
module privateendpoint '../Network/PrivateEndpointNoDNS.bicep' = {
  ...
  params: {
    ...
    subnetid: resourceId(subscription().id,first(split(peRGVnetSubnet, '/')), 'Microsoft.Network/virtualNetworks/subnets',substring(peRGVnetSubnet, indexOf(peRGVnetSubnet, '/') + 1, (lastIndexOf(peRGVnetSubnet, '/') - indexOf(peRGVnetSubnet, '/')) -1) , last(split(peRGVnetSubnet, '/')))
  }
}
